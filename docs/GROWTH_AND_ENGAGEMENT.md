...
| Mechanic | Status | Where | Note |
|---|---|---|---|
| **Referral — give-a-month / get-a-month** | **[UI/model only — no attribution]** | Model `ReferralCodeSchema` (`apps/web/src/lib/models.ts`), collection `referral_codes` (`db.ts`), screen `apps/ios/Arcaevo/Views/AccountV3/InviteV3View.swift` | Polished invite screen (NAME-XX code, ShareLink, `arcaevo.com/join?ref=<code>`). **But there is no referral API route, and `apps/web/src/app/join/JoinForm.tsx` ignores `?ref=` entirely** — so a referral is never attributed and **no free month is ever credited to either side.** Counts are hardcoded demo values. This is the biggest single PLG gap. |
...
| 3 | **Make referral actually work** | 5 | M | The screen and model exist; **wire the backend**: a referral API route, make `JoinForm.tsx` read + persist `?ref=`, attribute the join to the code (`joinedCount`), and credit **both sides** a real free month on activation (`freeMonthsApplied` → a Stripe coupon/trial extension). Today it rewards nobody. This is the single biggest PLG unlock and the copy already promises it. |
...
3. **Make referral actually attribute + reward.** Wire the API + `?ref=` persistence + a real both-sides free month. Today the polished invite screen rewards nobody — biggest PLG unlock, promise already made.
