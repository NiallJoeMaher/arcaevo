...
MEMBER APP   dashboard · readiness (blood-recalibrated, cycle-aware) · energy (all-day gauge) ·
             morning check-in · vitality age (chronic slow score) · fusion timeline ·
             results (clinician note on every panel) · marker detail (ApoB) ·
             insights · experiments (incl. €69 recheck loop) · start experiment ·
             "did it work?" verdict · ask Arcaevo (chat) · widgets & complications ·
             notification copy (all 12 pushes, final copy)
...
APPLE WATCH  watch face (readiness complication) · today — readiness · energy gauge ·
             felt check-in · vitality glance · biomarker glance · quick-log ·
             live workout (zone + ceiling bar) · active experiment · result ready
```

## Daily-engagement layer (July 2026 update)

The blood-recalibrated **Readiness score**, the **Energy** gauge, the **morning check-in**, the
**widgets/complications** surface, expanded **notification** prefs, and the new watch
readiness/energy/check-in screens are specified in **`ALGORITHM.md`** — read it before building
these. Core idea: blood modulates the *baseline and confidence band* of the daily score itself
(the fusion no competitor ships), deterministic rules decide, AI only narrates. The `readiness`
screen's blood-layer ON/OFF toggle (71 → 62) is a real transparency feature, not a demo prop.
ALGORITHM.md also specifies **Vitality Age** (RCV-gated chronic score), **cycle-aware baselines**,
the **clinician note on every panel**, the **€69 recheck loop** (the only monetisation surface —
never supplements), and the full notification vocabulary (final copy on the `pushgallery` screen).
...
- Notification prefs (8 toggles incl. Face ID lock; readiness/vitals/monthly on, focus/energy-dip off by default)
