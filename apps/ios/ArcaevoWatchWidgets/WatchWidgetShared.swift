import SwiftUI

// MARK: - Glance snapshot DTO (App-Group read-only mirror, watch side)
//
// The watch app writes `GlanceSnapshot` (ArcaevoKit/SnapshotStore) to the
// shared App-Group container on the WATCH after each compute; this complication
// extension reads it. Lightweight — links nothing heavy — so it carries a local
// Codable mirror with the SAME field names + ISO-8601 dates. Keep in sync with
// ArcaevoKit's GlanceSnapshot (schema owner: Wave 1a).

struct GlanceDTO: Codable, Hashable {
    var readiness: Int?
    var state: String
    var decision: String
    var energy: Int?
    var nextTestDays: Int?
    var updatedAt: Date
    var calibrationDay: Int?
    var calibrationOf: Int?
}

enum GlanceReader {
    static let appGroupID = "group.co.arcaevo.app"
    static let fileName = "glance-snapshot.json"

    private static let decoder: JSONDecoder = {
        let d = JSONDecoder()
        d.dateDecodingStrategy = .iso8601
        return d
    }()

    static func read() -> GlanceDTO? {
        guard
            let container = FileManager.default.containerURL(
                forSecurityApplicationGroupIdentifier: appGroupID
            )
        else { return nil }
        let url = container.appendingPathComponent(fileName)
        guard let data = try? Data(contentsOf: url) else { return nil }
        return try? decoder.decode(GlanceDTO.self, from: data)
    }

    static let placeholder = GlanceDTO(
        readiness: 62,
        state: "ok",
        decision: "goEasy",
        energy: 54,
        nextTestDays: 12,
        updatedAt: Date(),
        calibrationDay: nil,
        calibrationOf: nil
    )
}

enum Decision: String {
    case trainHard, trainAsPlanned, goEasy, rest
    init(_ raw: String) { self = Decision(rawValue: raw) ?? .rest }

    var short: String {
        switch self {
        case .trainHard: return "Train hard"
        case .trainAsPlanned: return "As planned"
        case .goEasy: return "Go easy"
        case .rest: return "Rest"
        }
    }

    var ceiling: Int {
        switch self {
        case .trainHard: return 9
        case .trainAsPlanned: return 7
        case .goEasy: return 4
        case .rest: return 1
        }
    }

    var accent: Color {
        switch self {
        case .trainHard, .trainAsPlanned: return WColor.green
        case .goEasy, .rest: return WColor.amber
        }
    }
}

enum WColor {
    static func hex(_ hex: UInt32) -> Color {
        Color(
            .sRGB,
            red: Double((hex >> 16) & 0xFF) / 255,
            green: Double((hex >> 8) & 0xFF) / 255,
            blue: Double(hex & 0xFF) / 255,
            opacity: 1
        )
    }
    static let green = hex(0x34A07C)
    static let brightGreen = hex(0x7FD3AE)
    static let amber = hex(0xD99A4E)
    static let gold = hex(0xD9C9A4)
    static let cream = hex(0xF4F1EA)
    static let mutedOnDark = hex(0x8FA89A)
}

extension GlanceDTO {
    var isCalibrating: Bool { state == "calibrating" }
    var isSparse: Bool { state == "sparseNight" }
    var decisionModel: Decision { Decision(decision) }
    var ringAccent: Color { isCalibrating ? WColor.gold : decisionModel.accent }

    var ringFraction: Double {
        if isCalibrating, let d = calibrationDay, let of = calibrationOf, of > 0 {
            return Double(d) / Double(of)
        }
        return Double(readiness ?? 0) / 100
    }

    var ringGlyph: String {
        if isCalibrating { return "\(calibrationDay ?? 0)/\(calibrationOf ?? 28)" }
        if isSparse || readiness == nil { return "—" }
        return "\(readiness!)"
    }

    var statusLine: String {
        if isCalibrating { return "Calibrating" }
        if isSparse { return "No read" }
        if state == "sick" { return "Rest — recover" }
        return decisionModel.short
    }

    var accessibilityLabel: String {
        if isCalibrating { return "Readiness calibrating, day \(calibrationDay ?? 0) of \(calibrationOf ?? 28)" }
        if isSparse { return "Readiness unavailable, no reading last night" }
        if let r = readiness { return "Readiness \(r), \(decisionModel.short)" }
        return statusLine
    }
}
