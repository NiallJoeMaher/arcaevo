...
*(`ClinicianNoteSchema`; `models.ts` `composeClinicianNote`; `STRATEGY.md` §3 #2)*
...
- **Two separate red lines here:**
  1. **Regulatory (device):** the *app* must not present the note as an automated diagnostic output — and it does not: the note is framed as a wellness summary and steers medical questions to the GP.
  2. **Clinical governance (fraud/safety):** presenting a fabricated clinician name/IMC, or auto-"reviewing" a real result with no human, is a serious clinical-governance and potential fraud issue. Real IMC-registered clinician + medical-ops partner before any real blood result — see `docs/LAUNCH_READINESS.md` §3.
- **RESOLVED — not-yet-real, framed as an automated summary (GAP_REVIEW_2 #2, 2026-07-05):** because **no registered reviewing clinician is onboarded yet**, the note (and the GP-share summary at `/s/[token]` and the E6/E7 emails) is now presented as an **automated wellness summary — not a diagnosis and not a clinician sign-off**, with a visible GP-facing disclaimer, and states that a registered clinician reviews results on the blood-testing tiers **once one is onboarded**. `composeClinicianNote` defaults to this framing and stamps **no** clinician name/IMC (empty strings); a real name + IMC is rendered **only** when a real registered clinician is passed in (data-driven). The fictional "Dr. S. Nolan, IMC 412887" persona was renamed to `DEMO_CLINICIAN_*` and is **demo/test-only — never written into seeded data and never shown to a real user or GP as a real review**. **A registered clinician remains a hard gate for the paid blood tiers.**
- **Wording for the note itself:** once a real clinician is speaking the note may legitimately be clinical — but the **app framing** around it should stay wellness, and the note must still steer medical questions to the user's GP.
- **Verdict:** the *software* stays out of scope; the *service* is a clinical activity that must be governed. Low device-risk; the mock-persona governance risk is closed for now by the honest automated-summary framing, and re-opens only when a real clinician is onboarded (paid tiers).
