...
|---|---|---|
| Controller (interim) | **Codú Limited**, Ireland — the founder's existing registered Irish company, acting as **interim data controller for the early closed trial** | **DECISION (founder, 2026-07):** Codú Limited is the controller for the trial phase. A **dedicated entity will be formed if the product monetises**, at which point the controller migrates and this register + the public privacy copy are updated. **OPEN: confirm Codú Limited's CRO number is recorded here, and that the public privacy copy is reconciled** — `apps/web/src/content/legal.ts` currently still asserts "Arcaevo Health Ltd" (verbatim design copy). Until reconciled, add a short interim-controller note to the privacy page (design-copy change) or a covering notice to trial users naming Codú Limited. |
| Controller representative | Founder (Niall Maher), for Codú Limited | Confirm |
| Data Protection Officer | **None appointed** | **OPEN: a DPO is likely required — Art. 37(1)(c), core activities = large-scale special-category processing. Either appoint one or document the decision + name a privacy contact.** At trial scale (<10 users) a documented "no DPO required, named privacy contact instead" decision is defensible — record it. Privacy contact address in copy: `privacy@arcaevo.health` (`legal.ts`, data-deletion doc). **Domain reconciliation:** app/marketing host and SES sending domain is `arcaevo.com` (`infra/cdk/SES_SETUP.md`); the iOS bundle prefix implies `arcaevo.co`; privacy copy uses `arcaevo.health`; interim founder contact is `niall@codu.ie`. For the trial, use **a mailbox on a subdomain of `arcaevo.com`** (e.g. `privacy@arcaevo.com`, deliverable via the SES-verified domain) and/or **`niall@codu.ie`** as the interim controller contact — then align every legal document and the public copy to whichever is chosen. |
