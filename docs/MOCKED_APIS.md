...
- **Still to productionise**: real EU Stripe entity + live keys (swap to a **restricted `rk_` key** for the server); enable Stripe Tax + IE VAT registration; verify the Apple Pay domain; configure the **Customer Portal** (upgrade/downgrade/cancel — Arcaevo's +€130 quarterly upgrade & cancel-renewal); real refund path (we currently apply only the policy — no charge id is persisted per order).
