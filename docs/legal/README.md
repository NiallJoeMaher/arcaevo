...
2. **Solicitor-review** the privacy policy / consent copy / terms / sub-processor page, and **confirm the interim controller entity** (**Codú Limited**, CRO number — placeholder `[TODO: CRO number]` throughout). **Company attribution is set to Codú Limited and the privacy policy + terms are hardened** for the Art. 9 launch (pending solicitor sign-off): the privacy policy now covers controller identity, special-category (Art. 9) data, Art. 6 + Art. 9(2)(a) explicit consent (versioned, withdrawable), purposes, named sub-processors (Atlas/Vercel/SES/Stripe/PostHog — EU/SCCs), international transfers, retention + the real erasure flow, all data-subject rights, the **right to complain to the DPC**, an Art. 22 automated-processing note, cookies/analytics, security, children (18+), and change/versioning — plus the wellness-not-diagnosis disclaimer. Public copy carries the role-based **`privacy@arcaevo.com`** contact and the "Arcaevo is a product of Codú Limited, registered in Ireland" attribution in the footer, privacy/terms, `/legal/[doc]`, `/contact` and `/about`. The **DPO decision** is captured as a documented "not required (yet)" position in [`DPO_NOT_REQUIRED_MEMO.md`](./DPO_NOT_REQUIRED_MEMO.md) — **needs sign-off**, and a DPO **will** be appointed at launch scale.
