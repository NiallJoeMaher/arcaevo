...
- **Magic-link auth is resilient to email link-prefetching (Phase 21).** Magic-link is the PRIMARY auth (email-only, no SIWA), so two independent defences guard against email virus-scanners (Safe Links, Mimecast, Proofpoint) prefetching the link and burning the single-use token: (1) `/verify` does NOT auto-redeem the token on page load — a "Confirm sign-in" button POSTs it only on a human tap (a prefetcher that follows the link but doesn't submit can't consume it); (2) every verify/magic-link email ALSO carries a typed 6-char code (`XXX-XXX`) that redeems the SAME single-use token via `{email, code}` — a scanner never fills in and submits a code field. The code is hash-only, email-scoped, 30-min, single-use, capped at 5 attempts. Assumption: the code narrows brute-force to ~5 guesses per issued token but does NOT replace real IP/global rate-limiting on the verify endpoint (still a launch TODO, below).
...
- **Device-scoped watch sessions (golden watch login).** The Apple Watch authenticates independently of the phone: an authenticated member mints a `device:"watch"` session (`POST /api/v1/auth/watch-session`) — a fresh token with its own DB row, never a copy of the phone token, individually revocable. Assumptions: **one active watch session per user** (minting revokes the prior watch session — replace policy, chosen so a lost/re-paired watch can't accumulate live credentials); sessions carry a **30-day sliding TTL** (`SESSION_TTL_DAYS`) refreshed on `POST /api/v1/auth/session/refresh` (the watch's silent refresh on 401/wake); pre-existing sessions without `expiresAt`/`device` are treated as non-expiring `web` sessions (backward compat). The watch token is a real credential — same SHA-256 hashing, never logged, prod env gating applies — and authenticates consent-guarded health endpoints via the existing member-auth path once it holds a valid, consented session.
- **Golden watch login — iOS/watch client (assumptions).** The phone hands the watch token over via `WCSession.updateApplicationContext` (latest-state-wins; delivered on the watch's next wake even if unreachable). This assumes the watch app is **embedded + paired** (the `ArcaevoWatch` target dependency provides pairing; WCSession needs no entitlement). The watch stores its token in **its own keychain** (no App Group / shared keychain exists between the devices — that's *why* the handoff is over WatchConnectivity) with `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` — a deliberate choice over plain `AfterFirstUnlock` (still allows background/LTE refresh after first unlock, but keeps a device-scoped, revocable credential out of the iCloud keychain). On a transient (non-401) refresh failure the watch stays optimistically authenticated so an LTE blip with the phone away doesn't bounce the member to the setup screen; only a real 401 (from refresh, or from one silent retry after a data-call 401) clears the token. **No Sign in with Apple**: the product is email + magic-link only at launch, and ASWebAuthenticationSession doesn't exist on watchOS — so a **watch-only user cannot sign in on the wrist**; the "Finish setup on your iPhone" screen is the deliberate fallback and sign-in must happen once on the paired iPhone. DEBUG builds keep a demo fallback (lone simulator gets no WC context → seeded demo data); Release never fabricates a token and shows the setup screen when unauthenticated.

### Still assumed / must be done before real launch

- Real IdP-grade rate limiting (IP + global) — still outstanding; the magic-link cool-off + the Phase-21 5-attempt code ceiling narrow abuse but do not replace it (especially on `POST /auth/magic-link/verify`). Passkeys/TOTP (designed as "coming"), Sign in with Apple later.
